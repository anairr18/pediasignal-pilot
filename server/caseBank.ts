...
function convertAliEmToCaseDefinition(aliEmCase: any): CaseDefinition {
  ...
  return {
    ...
    difficulty: aliEmCase.difficulty || (variant.stages[0]?.severity === 'critical' ? 'advanced' : 'intermediate'),
    ...
    stages: variant.stages.map((stage: any, index: number) => ({
      ...
      ordered: stage.ordered || false,
      ageBand: variant.ageBand || 'child',
      TTIsec: stage.TTIsec || 60,
      vitalBounds: {
      ...
      vitals: {
        ...
        temperature: variant.initialVitals.temperature || 98.6,
        respRate: variant.initialVitals.respRate || 20,
        spo2: variant.initialVitals.spo2 || 98,
        bloodGlucose: variant.initialVitals.bloodGlucose || null,
        consciousness: variant.initialVitals.consciousness || 'alert',
        bloodPressureSys: variant.initialVitals.bloodPressureSys || 90,
        bloodPressureDia: variant.initialVitals.bloodPressureDia || 60,
        capillaryRefill: variant.initialVitals.capillaryRefill || 2
      },
    ...
    description: aliEmCase.description || `ALiEM Case: ${aliEmCase.displayName} - ${aliEmCase.sourceCitation}`
  };
}
...
function loadAliEmCases(): CaseDefinition[] {
  try {
    // Import ALiEM cases from the generated JSON file
    const aliEmCasesPath = path.join(process.cwd(), 'server', 'data', 'caseBank.aliem.json');
    console.log(`Attempting to load ALiEM cases from: ${aliEmCasesPath}`);
    
    if (fs.existsSync(aliEmCasesPath)) {
      const aliEmData = fs.readFileSync(aliEmCasesPath, 'utf8');
      const aliEmCases = JSON.parse(aliEmData);
      
      console.log(`✅ Loaded ${aliEmCases.length} ALiEM cases from ${aliEmCasesPath}`);
      
      // Convert ALiEM cases to CaseDefinition format
      const convertedCases = aliEmCases.map((aliEmCase: any) => convertAliEmToCaseDefinition(aliEmCase));
      console.log(`✅ Converted ${convertedCases.length} ALiEM cases to CaseDefinition format`);
      
      return convertedCases;
    } else {
      console.log(`⚠️ ALiEM cases file not found at ${aliEmCasesPath}`);
      ...
      try {
        ...
        if (fs.existsSync(altPath)) {
          ...
          const aliEmCases = JSON.parse(aliEmData);
          console.log(`✅ Loaded ${aliEmCases.length} ALiEM cases from alternative path: ${altPath}`);
          ...
          console.log(`⚠️ ALiEM cases file not found at alternative path either: ${altPath}, using legacy cases`);
    return [];
        }
              } catch (error) {
          console.log('Could not determine alternative path:', (error as Error).message);
          return [];
        }
    }
  } catch (error) {
...
export const interventions: Record<string, Intervention> = {
  ...
  // ALiEM ANPHYLAXIS INTERVENTIONS

  "IM epinephrine": {
    id: "IM epinephrine",
            name: "IM Epinephrine",
    description: "Intramuscular epinephrine 0.01 mg/kg (max 0.5 mg) for severe anaphylaxis. Based on PALS 2020 anaphylaxis guidelines.",
    category: "medication",
    timeRequired: 30,
    successRate: 0.95,
    vitalEffects: {
      heartRate: { immediate: -10, delayed: -15 },
      respRate: { immediate: -4, delayed: -8 },
      bloodPressure: { immediate: 10, delayed: 15 },
      oxygenSat: { immediate: 2, delayed: 5 }
    },
    ragSummary: "First-line treatment for severe anaphylaxis",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  "IV fluids bolus": {
    id: "IV fluids bolus",
            name: "IV Fluids Bolus",
    description: "Rapid IV fluid administration (20 mL/kg) for hypotension in anaphylaxis. Based on PALS 2020 fluid resuscitation guidelines.",
    category: "procedure",
    timeRequired: 120,
    successRate: 0.90,
    vitalEffects: {
      bloodPressure: { immediate: 5, delayed: 10 },
      heartRate: { immediate: -5, delayed: -10 }
    },
    ragSummary: "Volume expansion for hypotension",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "diphenhydramine IV": {
    id: "diphenhydramine IV",
            name: "Diphenhydramine IV",
    description: "IV diphenhydramine 1 mg/kg (max 50 mg) for histamine blockade in anaphylaxis. Based on PALS 2020 guidelines.",
    category: "medication",
    timeRequired: 60,
    successRate: 0.90,
    vitalEffects: {
      heartRate: { immediate: -5, delayed: -10 },
      respRate: { immediate: -2, delayed: -5 }
    },
    ragSummary: "Histamine blockade for anaphylaxis",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "H2 blocker IV": {
    id: "H2 blocker IV",
            name: "H2 Blocker IV",
    description: "IV ranitidine 1 mg/kg (max 50 mg) for additional histamine blockade. Based on PALS 2020 anaphylaxis guidelines.",
    category: "medication",
    timeRequired: 60,
    successRate: 0.90,
    vitalEffects: {
      heartRate: { immediate: -3, delayed: -5 },
      respRate: { immediate: -1, delayed: -3 }
    },
    ragSummary: "Additional histamine blockade",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "nebulized beta-agonist": {
    id: "nebulized beta-agonist",
            name: "Nebulized Beta-Agonist",
    description: "Albuterol 2.5 mg nebulized for bronchospasm in anaphylaxis. Based on PALS 2020 respiratory guidelines.",
    category: "medication",
    timeRequired: 300,
    successRate: 0.85,
    vitalEffects: {
      respRate: { immediate: -5, delayed: -10 },
      oxygenSat: { immediate: 3, delayed: 8 }
    },
    ragSummary: "Bronchodilation for bronchospasm",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "steroids IV": {
    id: "steroids IV",
            name: "IV Steroids",
    description: "IV methylprednisolone 2 mg/kg (max 125 mg) for delayed anaphylaxis prevention. Based on PALS 2020 guidelines.",
    category: "medication",
    timeRequired: 60,
    successRate: 0.90,
    vitalEffects: {
    ...
    ragSummary: "Delayed anaphylaxis prevention",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "delay epinephrine": {
    id: "delay epinephrine",
            name: "Delay Epinephrine",
    description: "Delaying epinephrine administration in anaphylaxis worsens outcomes. Based on PALS 2020 critical timing guidelines.",
    category: "medication",
    timeRequired: 0,
    successRate: 0.00,
    vitalEffects: {
      bloodPressure: { immediate: -8, delayed: -15 },
      oxygenSat: { immediate: -2, delayed: -8 },
      consciousness: { immediate: -1, delayed: -3 }
    },
    ragSummary: "Delaying epinephrine worsens outcomes",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "epinephrine PO": {
    id: "epinephrine PO",
            name: "Oral Epinephrine",
    description: "Oral epinephrine is ineffective and potentially harmful. Must use IM route. Based on PALS 2020 guidelines.",
    category: "medication",
    timeRequired: 0,
    successRate: 0.00,
    vitalEffects: {
    ...
    ragSummary: "Oral epinephrine is ineffective",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "unnecessary intubation without indications": {
    id: "unnecessary intubation without indications",
            name: "Unnecessary Intubation",
    description: "Intubation without proper indications can cause harm. Based on PALS 2020 airway management guidelines.",
    category: "procedure",
    timeRequired: 0,
    successRate: 0.00,
    vitalEffects: {
      consciousness: { immediate: -2, delayed: -5 },
      respRate: { immediate: -5, delayed: -10 }
    },
    ragSummary: "Unnecessary intubation causes harm",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "CBC": {
    ...
    description: "Laboratory test for complete blood count. Not critical in acute anaphylaxis management.",
    category: "monitoring",
    timeRequired: 180,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Laboratory test - not critical acutely",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "CXR (normal)": {
    id: "CXR (normal)",
    name: "Chest X-Ray",
    description: "Chest radiograph showing normal findings. Not critical in acute anaphylaxis management.",
    category: "monitoring",
    timeRequired: 300,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Imaging study - not critical acutely",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "continuous monitoring": {
    ...
    description: "Continuous vital signs and cardiac monitoring for anaphylaxis patients. Based on PALS 2020 guidelines.",
    category: "monitoring",
    timeRequired: 0,
    successRate: 1.00,
    vitalEffects: {},
    ragSummary: "Continuous monitoring for anaphylaxis",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  "repeat epinephrine if needed": {
    id: "repeat epinephrine if needed",
    name: "Repeat Epinephrine if Needed",
    description: "Repeat IM epinephrine every 5-15 minutes if symptoms persist. Based on PALS 2020 anaphylaxis guidelines.",
    category: "medication",
    timeRequired: 30,
    successRate: 0.95,
    vitalEffects: {
    ...
    ragSummary: "Repeat epinephrine for persistent symptoms",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "antihistamines": {
    id: "antihistamines",
            name: "Antihistamines",
    description: "Oral or IV antihistamines for symptom relief. Based on PALS 2020 anaphylaxis guidelines.",
    category: "medication",
    timeRequired: 60,
    successRate: 0.90,
    vitalEffects: {
    ...
    ragSummary: "Symptom relief with antihistamines",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "beta-agonists": {
    id: "beta-agonists",
            name: "Beta-Agonists",
    description: "Inhaled beta-agonists for persistent bronchospasm. Based on PALS 2020 respiratory guidelines.",
    category: "medication",
    timeRequired: 300,
    successRate: 0.85,
    vitalEffects: {
    ...
    ragSummary: "Persistent bronchospasm treatment",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Recognition & ABCs", passageId: 1, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  "discharge before 4 hours": {
    id: "discharge before 4 hours",
            name: "Discharge Before 4 Hours",
    description: "Discharging anaphylaxis patients before 4 hours of monitoring is harmful. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 0,
    successRate: 0.00,
    vitalEffects: {
    ...
    ragSummary: "Early discharge is harmful",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "missing follow-up plan": {
    id: "missing follow-up plan",
            name: "Missing Follow-up Plan",
    description: "Failing to provide follow-up care plan for anaphylaxis patients is harmful. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 0,
    successRate: 0.00,
    vitalEffects: {
      consciousness: { immediate: 0, delayed: -1 },
      bloodPressure: { immediate: 0, delayed: -2 }
    },
    ragSummary: "Missing follow-up plan is harmful",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "allergy testing": {
    ...
    description: "Allergy testing for anaphylaxis trigger identification. Not critical in acute management.",
    category: "monitoring",
    timeRequired: 600,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Allergy testing - not critical acutely",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "discharge instructions": {
    ...
    description: "Provide discharge instructions and epinephrine auto-injector prescription. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 120,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Discharge instructions and epinephrine prescription",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  // MISSING ANAPHYLAXIS INTERVENTIONS

  "reassess vitals": {
    id: "reassess vitals",
    name: "Reassess Vital Signs",
    description: "Reassess vital signs to monitor response to treatment. Based on PALS 2020 guidelines.",
    category: "monitoring",
    timeRequired: 60,
    successRate: 1.0,
    vitalEffects: {},
    ragSummary: "Reassess vital signs for treatment response",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  "document response": {
    id: "document response",
    name: "Document Response to Treatment",
    description: "Document patient's response to treatment and interventions. Based on medical documentation standards.",
    category: "monitoring",
    timeRequired: 60,
    successRate: 1.0,
    vitalEffects: {},
    ragSummary: "Document patient response to treatment",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "oral antihistamines": {
    ...
    description: "Oral antihistamines for additional histamine blockade after acute treatment. Based on PALS 2020 guidelines.",
    category: "medication",
    timeRequired: 60,
    successRate: 0.9,
    vitalEffects: {
    ...
    ragSummary: "Additional histamine blockade with oral antihistamines",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "discharge without observation": {
    id: "discharge without observation",
            name: "Discharge Without Observation",
    description: "Discharging anaphylaxis patients without adequate observation period is harmful. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 0,
    successRate: 0,
    vitalEffects: {
    ...
    ragSummary: "Premature discharge is harmful in anaphylaxis",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Initial Therapy & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  "inadequate monitoring": {
    id: "inadequate monitoring",
            name: "Inadequate Monitoring",
    description: "Inadequate monitoring in anaphylaxis can lead to missed deterioration. Based on PALS 2020 guidelines.",
    category: "monitoring",
    timeRequired: 0,
    successRate: 0,
    vitalEffects: {
      consciousness: { immediate: -1, delayed: -2 },
      bloodPressure: { immediate: -3, delayed: -8 }
    },
    ragSummary: "Inadequate monitoring causes harm",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Initial Therapy & Monitoring", passageId: 2, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "premature discharge": {
    id: "premature discharge",
            name: "Premature Discharge",
    description: "Premature discharge without adequate observation is harmful in anaphylaxis. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 0,
    successRate: 0,
    vitalEffects: {
    ...
    ragSummary: "Premature discharge is harmful",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "inadequate follow-up": {
    id: "inadequate follow-up",
            name: "Inadequate Follow-up",
    description: "Inadequate follow-up care planning for anaphylaxis patients is harmful. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 0,
    successRate: 0,
    vitalEffects: {
    ...
    ragSummary: "Inadequate follow-up planning is harmful",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "allergy referral": {
    id: "allergy referral",
            name: "Allergy Referral",
    description: "Refer patient to allergist for comprehensive evaluation. Based on AAP allergy guidelines.",
    category: "procedure",
    timeRequired: 120,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Allergy referral for comprehensive evaluation",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },
  ...
  "epinephrine auto-injector prescription": {
    id: "epinephrine auto-injector prescription",
            name: "Epinephrine Auto-Injector Prescription",
    description: "Prescribe epinephrine auto-injector and provide training. Based on PALS 2020 guidelines.",
    category: "procedure",
    timeRequired: 180,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Prescribe and train on epinephrine auto-injector",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Reassessment & Escalation", passageId: 3, sourceCitation: "ALiEM EM ReSCu Peds - Case 1: Anaphylaxis", license: "CC BY-NC-SA 4.0" }
    ]
  },

  // STAGE 4 DISCHARGE & FOLLOW-UP INTERVENTIONS (Anaphylaxis)
  "comprehensive discharge instructions": {
    id: "comprehensive discharge instructions",
    name: "Comprehensive Discharge Instructions",
    description: "Provide detailed discharge instructions including medication schedules, warning signs, emergency contacts, and follow-up care. Based on PALS 2020 guidelines.",
    category: "supportive",
    timeRequired: 300,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Provide comprehensive discharge instructions for anaphylaxis patients",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 1, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "allergist referral scheduling": {
    id: "allergist referral scheduling",
    name: "Allergist Referral Scheduling",
    description: "Schedule follow-up appointment with pediatric allergist for comprehensive evaluation and long-term management planning. Based on AAP allergy guidelines.",
    category: "supportive",
    timeRequired: 120,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Schedule allergist referral for comprehensive evaluation",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 2, sourceCitation: "AAP Allergy Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "school action plan distribution": {
    id: "school action plan distribution",
    name: "School Action Plan Distribution",
    description: "Create and distribute comprehensive allergy action plan to school including emergency protocols, medication administration, and contact information. Based on school health guidelines.",
    category: "supportive",
    timeRequired: 240,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Distribute school action plan for allergy management",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 3, sourceCitation: "School Health Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "follow-up appointment booking": {
    id: "follow-up appointment booking",
    name: "Follow-up Appointment Booking",
    description: "Schedule follow-up appointment with primary care provider for continued monitoring and care coordination. Essential for ongoing anaphylaxis management.",
    category: "supportive",
    timeRequired: 90,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Book follow-up appointment for continued care",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 4, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },
  ...
  "patient education": {
    ...
    description: "Comprehensive patient and family education on anaphylaxis recognition, prevention, and emergency response. Includes trigger avoidance and emergency action steps.",
    category: "supportive",
    timeRequired: 360,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Provide comprehensive patient education on anaphylaxis",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 5, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "missing epinephrine prescription": {
    id: "missing epinephrine prescription",
            name: "Missing Epinephrine Prescription",
    description: "Failing to prescribe epinephrine auto-injector is harmful and increases risk of fatal anaphylaxis. Based on PALS 2020 guidelines.",
    category: "supportive",
    timeRequired: 0,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Missing epinephrine prescription is harmful",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 6, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "inadequate discharge instructions": {
    id: "inadequate discharge instructions",
            name: "Inadequate Discharge Instructions",
    description: "Providing inadequate discharge instructions can lead to poor outcomes and increased readmission risk. Comprehensive instructions are essential.",
    category: "supportive",
    timeRequired: 0,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Inadequate discharge instructions are harmful",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 7, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "allergy testing scheduling": {
    id: "allergy testing scheduling",
            name: "Allergy Testing Scheduling",
    description: "Schedule comprehensive allergy testing for trigger identification and long-term management planning. Not critical for acute management but helpful for prevention.",
    category: "supportive",
    timeRequired: 180,
    successRate: 0.95,
    vitalEffects: {},
    ragSummary: "Schedule allergy testing for trigger identification",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 8, sourceCitation: "AAP Allergy Guidelines", license: "CC-BY-4.0" }
    ]
  },

  "home monitoring instructions": {
    id: "home monitoring instructions",
            name: "Home Monitoring Instructions",
    description: "Provide instructions for home monitoring including vital signs, symptoms to watch for, and when to seek emergency care. Supports safe discharge.",
    category: "supportive",
    timeRequired: 120,
    successRate: 0.90,
    vitalEffects: {},
    ragSummary: "Provide home monitoring instructions for safe discharge",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Discharge & Follow-up", passageId: 9, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },
  ...
  "vital_signs_monitoring": {
    ...
    description: "Continuous monitoring of heart rate, respiratory rate, blood pressure, oxygen saturation, and temperature. Essential for detecting deterioration in anaphylaxis. Based on PALS 2020 guidelines.",
    category: "monitoring",
    timeRequired: 60,
    successRate: 0.95,
    vitalEffects: {
    ...
    ragSummary: "Continuous vital signs monitoring for anaphylaxis patients",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 1, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  oxygen_therapy_maintenance: {
    id: "oxygen_therapy_maintenance",
    name: "Oxygen Therapy Maintenance",
    description: "Maintain supplemental oxygen therapy to ensure adequate oxygenation. Monitor SpO2 and adjust flow rate as needed. Based on PALS 2020 anaphylaxis protocols.",
    category: "monitoring",
    timeRequired: 45,
    successRate: 0.90,
    vitalEffects: {
      oxygenSat: { immediate: 2, delayed: 3 },
      respRate: { immediate: -1, delayed: -2 },
      heartRate: { immediate: -2, delayed: -3 }
    },
    ragSummary: "Maintain oxygen therapy for anaphylaxis stabilization",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 2, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  iv_access_assessment: {
    id: "iv_access_assessment",
    name: "IV Access Assessment",
    description: "Assess and maintain IV access for potential medication administration. Check patency, securement, and flow rate. Critical for anaphylaxis management.",
    category: "monitoring",
    timeRequired: 30,
    successRate: 0.95,
    vitalEffects: {
    ...
    ragSummary: "Assess and maintain IV access for anaphylaxis management",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 3, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  capillary_refill_check: {
    id: "capillary_refill_check",
    name: "Capillary Refill Check",
    description: "Assess capillary refill time to evaluate peripheral perfusion. Normal refill time is <2 seconds. Prolonged refill indicates poor perfusion.",
    category: "monitoring",
    timeRequired: 20,
    successRate: 0.95,
    vitalEffects: {
    ...
    ragSummary: "Assess capillary refill for perfusion evaluation",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 4, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  },

  urine_output_monitoring: {
    id: "urine_output_monitoring",
    name: "Urine Output Monitoring",
    description: "Monitor urine output to assess renal perfusion and fluid status. Normal pediatric urine output is 1-2 mL/kg/hour. Decreased output may indicate hypovolemia.",
    category: "monitoring",
    timeRequired: 60,
    successRate: 0.90,
    vitalEffects: {
    ...
    ragSummary: "Monitor urine output for fluid status assessment",
    evidenceSources: [
      { caseId: "aliem_case_01_anaphylaxis", section: "Stabilization & Monitoring", passageId: 5, sourceCitation: "PALS 2020 Guidelines", license: "CC-BY-4.0" }
    ]
  }
...
// Additional Stage 2 interventions for Anaphylaxis - Stabilization & Monitoring
interventions["vital signs monitoring"] = {
  id: "vital signs monitoring",
  name: "Vital Signs Monitoring",
  description: "Continuous monitoring of heart rate, blood pressure, respiratory rate, oxygen saturation, and temperature. Essential for detecting deterioration in anaphylaxis. Based on PALS 2020 guidelines.",
  category: "monitoring",
  timeRequired: 30,
  successRate: 0.95,
  ragSummary: "Continuous vital sign monitoring is crucial in anaphylaxis management to detect deterioration or improvement.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
...
interventions["oxygen therapy maintenance"] = {
  id: "oxygen therapy maintenance",
  name: "Oxygen Therapy Maintenance",
  description: "Maintain supplemental oxygen to ensure adequate oxygenation. Monitor SpO2 and adjust FiO2 as needed.",
  category: "supportive",
  timeRequired: 30,
  successRate: 0.92,
  ragSummary: "Continued oxygen support maintains adequate oxygenation during anaphylaxis recovery.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
  ...
  vitalEffects: {
    oxygenSat: { immediate: 2, delayed: 1 }
  }
};

interventions["IV access assessment"] = {
  id: "IV access assessment",
  name: "IV Access Assessment",
  description: "Verify and maintain IV access for continued medication administration. Ensure patency and adequate flow rate.",
  category: "procedure",
  timeRequired: 60,
  successRate: 0.88,
  ragSummary: "Maintaining IV access is critical for ongoing anaphylaxis treatment and potential additional medications.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
...
interventions["capillary refill check"] = {
  id: "capillary refill check",
  name: "Capillary Refill Check",
  description: "Assess peripheral perfusion by checking capillary refill time. Normal is <2 seconds in children.",
  category: "monitoring",
  timeRequired: 15,
  successRate: 0.95,
  ragSummary: "Capillary refill assessment helps evaluate circulatory status and response to treatment.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
...
interventions["urine output monitoring"] = {
  id: "urine output monitoring",
  name: "Urine Output Monitoring",
  description: "Monitor urine output to assess kidney perfusion and fluid balance. Normal is >1 mL/kg/hr in children.",
  category: "monitoring",
  timeRequired: 30,
  successRate: 0.90,
  ragSummary: "Urine output monitoring helps assess renal perfusion and overall fluid status.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
...
interventions["albuterol nebulizer"] = {
  id: "albuterol nebulizer",
  name: "Albuterol Nebulizer",
  description: "Nebulized albuterol 2.5-5 mg for bronchospasm relief. Monitor for tachycardia and tremors.",
  category: "medication",
  timeRequired: 180,
  successRate: 0.85,
  ragSummary: "Albuterol helps relieve bronchospasm and improve respiratory symptoms in anaphylaxis.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
  ...
  vitalEffects: {
    respRate: { immediate: -3, delayed: -2 },
    oxygenSat: { immediate: 3, delayed: 2 },
    heartRate: { immediate: 5, delayed: 2 }
  }
};
...
interventions["discharge planning"] = {
  ...
  description: "Begin planning for discharge including education, follow-up arrangements, and prescription for epinephrine auto-injector.",
  category: "supportive",
  timeRequired: 120,
  successRate: 0.95,
  ragSummary: "Early discharge planning ensures proper follow-up and patient safety measures.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Stabilization & Monitoring",
    passageId: 2,
...
// Additional Stage 3 interventions for Anaphylaxis - Observation & Prevention
interventions["extended observation period"] = {
  id: "extended observation period",
  name: "Extended Observation Period",
  description: "Maintain patient under observation for adequate time to ensure stability and detect any delayed reactions. PALS guidelines recommend minimum 4-6 hours for anaphylaxis.",
  category: "monitoring",
  timeRequired: 240,
  successRate: 0.98,
  ragSummary: "Extended observation is critical in anaphylaxis to detect biphasic reactions and ensure patient stability before discharge.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Observation & Prevention",
    passageId: 3,
    sourceCitation: "ALiEM EM ReSCu Peds – Case 1: Anaphylaxis",
    license: "CC BY-NC-SA 4.0"
  }],
...
interventions["vital signs reassessment"] = {
  id: "vital signs reassessment",
  name: "Vital Signs Reassessment",
  description: "Comprehensive reassessment of all vital signs after initial treatment to evaluate response and detect any deterioration.",
  category: "monitoring",
  timeRequired: 45,
  successRate: 0.95,
  ragSummary: "Vital signs reassessment is essential to confirm treatment effectiveness and identify any ongoing issues.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Observation & Prevention",
    passageId: 3,
...
interventions["allergy action plan creation"] = {
  id: "allergy action plan creation",
  name: "Allergy Action Plan Creation",
  description: "Create a comprehensive allergy action plan for the patient and family, including trigger avoidance, recognition of symptoms, and emergency response steps.",
  category: "supportive",
  timeRequired: 90,
  successRate: 0.92,
  ragSummary: "A comprehensive allergy action plan is crucial for preventing future anaphylactic reactions and ensuring proper emergency response.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Observation & Prevention",
    passageId: 3,
...
interventions["epinephrine auto-injector training"] = {
  id: "epinephrine auto-injector training",
  name: "Epinephrine Auto-Injector Training",
  description: "Comprehensive training for patient and family on proper use of epinephrine auto-injector, including recognition of symptoms and administration technique.",
  category: "supportive",
  timeRequired: 120,
  successRate: 0.88,
  ragSummary: "Proper epinephrine auto-injector training is essential for patient safety and can prevent fatal outcomes in future reactions.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Observation & Prevention",
    passageId: 3,
...
interventions["discharge readiness assessment"] = {
  id: "discharge readiness assessment",
  name: "Discharge Readiness Assessment",
  description: "Comprehensive assessment to determine if patient is ready for discharge, including vital signs stability, symptom resolution, and family preparedness.",
  category: "monitoring",
  timeRequired: 60,
  successRate: 0.95,
  ragSummary: "Proper discharge readiness assessment ensures patient safety and prevents premature discharge that could lead to complications.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Observation & Prevention",
    passageId: 3,
...
interventions["inadequate follow-up planning"] = {
  id: "inadequate follow-up planning",
          name: "Inadequate Follow-up Planning",
  description: "Failing to establish proper follow-up care and monitoring after anaphylaxis treatment. This can lead to missed complications and poor long-term outcomes.",
  category: "supportive",
  timeRequired: 0,
  successRate: 0.10,
  ragSummary: "Inadequate follow-up planning is harmful and can result in missed complications, poor patient education, and increased risk of future severe reactions.",
  evidenceSources: [{
    caseId: "aliem_case_01_anaphylaxis",
    section: "Observation & Prevention",
    passageId: 3,
